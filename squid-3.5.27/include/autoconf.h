...
#define LT_DLSEARCH_PATH "/lib:/usr/lib:/usr/local/gcc/lib64:/usr/lib64/mysql"
...
#define PACKAGE_STRING "Squid Web Proxy 3.5.27"
...
#define SQUID_CONFIGURE_OPTIONS " '--prefix=/usr/local/squid' '--localstatedir=/usr/local/squid/var' '--sysconfdir=/usr/local/squid/etc' '--with-openssl=/data_ssd/shadow.tool/src/squid/squid-3.5.27/openssl-1.0.2n' '--enable-dlmalloc' '--enable-gnuregex' '--enable-carp' '--enable-async-io' '--enable-storeio=aufs,diskd,ufs' '--enable-icmp' '--enable-delay-pools' '--enable-removal-policies=heap,lru' '--enable-useragent-log' '--enable-referer-log' '--enable-snmp' '--enable-arp-acl' '--enable-htcp' '--enable-ssl' '--enable-cache-digests' '--with-coss-membuf-size=2097152' '--enable-poll' '--enable-linux-netfilter' '--enable-x-accelerator-vary' '--enable-stacktrace' '--enable-truncate' '--enable-underscores' '--enable-auth' --enable-ltdl-convenience"
